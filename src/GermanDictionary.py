...
dictionary_path = '../../../../../Library/Mobile Documents/com~apple~CloudDocs/Projects/Vocab Builder/German/Dictionary/Copilot Dictionary.json'
config_path = '../src/config/Vokabelbox/config.json'
notes_path = 'vocab notes.json'
...
#         with open('../src/config/Vokabelbox/config.json') as file:
...
#         with open('../src/config/Vokabelbox/config.json', 'w') as file:
...
class GermanDictionary:
    ...
    def update_dict(self, notes_path:str=notes_path, config_path:str=config_path):
        ...
        for word in notes:
            original_entry = self.dict.get(word, [])
            original_entry.append(notes[word])
            self.dict[word] = original_entry
            if word not in current_all_list:
                config['all'].append(word)
                config['new'].append(word)
